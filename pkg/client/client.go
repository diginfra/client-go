...
// it allows to connect to a Diginfra gRPC server.
...
// Outputs is the client for Diginfra Outputs.
// When using it you can use `Sub()` or `Get()` to receive a stream of Diginfra output events.
...
// OutputsWatch subscribes to a stream of Diginfra outputs
...
// Version it the client for Diginfra Version API.
// When using it you can use `version()` to receive the Diginfra version.
